...
VERSION ?= 0.1.0
ARM64_BIN_PATH := $(BUILD_DIR)/$(APP_NAME)-arm64
AMD64_BIN_PATH := $(BUILD_DIR)/$(APP_NAME)-amd64
UNIVERSAL_BIN_PATH := $(BUILD_DIR)/$(APP_NAME)
...
$(ARM64_BIN_PATH):
	mkdir -p $(BUILD_DIR)
	CGO_ENABLED=1 GOOS=$(TARGET_OS) GOARCH=arm64 go build -o $(ARM64_BIN_PATH) $(MODULE)

$(AMD64_BIN_PATH):
	mkdir -p $(BUILD_DIR)
	CGO_ENABLED=1 GOOS=$(TARGET_OS) GOARCH=amd64 go build -o $(AMD64_BIN_PATH) $(MODULE)

$(UNIVERSAL_BIN_PATH): $(ARM64_BIN_PATH) $(AMD64_BIN_PATH)
	lipo -create -output $(UNIVERSAL_BIN_PATH) $(ARM64_BIN_PATH) $(AMD64_BIN_PATH)
